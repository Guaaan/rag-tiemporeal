...
### Ejecución en una VM Linux usando tmux

Si deseas ejecutar el asistente en una máquina virtual Linux y mantenerlo corriendo en segundo plano, puedes usar `tmux` para gestionar sesiones persistentes.

#### Pasos sugeridos:

1. **Instala tmux** (si no está instalado):
   ```bash
   sudo apt update && sudo apt install tmux -y
   ```

2. **Conéctate a tu VM** (por ejemplo, usando SSH):
   ```bash
   ssh usuario@ip_de_tu_vm
   ```

3. **Crea una nueva sesión tmux**:
   ```bash
   tmux new -s asistente
   ```

4. **Navega al directorio del proyecto y activa tu entorno (si aplica)**:
   ```bash
   cd /ruta/al/proyecto
...
6. **Desconéctate de la sesión tmux sin detener la app**:
   - Presiona `Ctrl+B` y luego `D` para "detach".

7. **Para volver a la sesión más tarde**:
   ```bash
   tmux attach -t asistente
   ```

Esto permite que el asistente siga corriendo aunque cierres la terminal o la conexión SSH.

## Uso General

- Puedes interactuar con el asistente tanto por voz como por texto.
- El sistema soporta cambio de idioma y selección de voz.
- Permite búsquedas en bases de conocimiento y ejecución de herramientas externas.
